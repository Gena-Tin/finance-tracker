...
interface ColoredStatEntry extends StatEntry {
  fill: string;
}
...
const Analytics: React.FC<AnalyticsProps> = ({
  ...
  const coloredCategoryStats = useMemo<ColoredStatEntry[]>(() => {
  ...
  const timelineData = useMemo<TimelineGroup[]>(() => {
    ...
    const groups = filteredByCategory.reduce<Record<string, TimelineGroup>>(
      (acc, item) => {
        // Берем дату из 'created_at' и оставляем только часть YYYY-MM-DD
